<template>
  <div class="payStatus">
    <div class="suceess">
      <img src="@/assets/icon/home/success.png" alt />
      <div class="payText">支付完成</div>
      <div class="tips">
        您此单获得 {{getLb||0}} 乐币，评价后还可以额外获得
        <span>{{ewLb||0}} </span>乐币
      </div>
      <div class="btns">
...
<script>
import { appByStripeBack } from "@/api/home";
export default {
  data() {
    return {
      status: "",
  ...
  methods: {
    ...
    getAppByStripeBack() {
      ...
      appByStripeBack(data).then((res) => {
        if(res.code==1000){
          this.ewLb=res.data.coinComment
          this.getLb=parseInt(res.data.payPrice*res.data.coinOnline)
        }
      });
    },
    goHome() {
